...
const tokenTips: Record<string, { ru: string; en: string }> = {
  ...
  '-m': { ru: 'Флаг: указать сообщение или переименовать ветку', en: 'Flag: specify message or rename branch' },
  ...
  'gpg.format': { ru: 'Настройка: формат подписи (например ssh)', en: 'Setting: signature format (e.g. ssh)' },
  'user.signingkey': { ru: 'Настройка: путь к ключу для подписи коммитов', en: 'Setting: path to signing key for commits' },
  'commit.gpgsign': { ru: 'Настройка: подписывать все коммиты автоматически', en: 'Setting: auto-sign all commits' },
  'core.longpaths': { ru: 'Настройка: поддержка длинных путей (>260 символов)', en: 'Setting: support long paths (>260 chars)' },
  'core.editor': { ru: 'Настройка: какой редактор использовать для сообщений', en: 'Setting: which editor to use for messages' },
  ...
  // Git stash/bisect/remote subcommands
  'pop': { ru: 'Подкоманда stash: достать и удалить из хранилища', en: 'Stash subcommand: apply and remove from stash' },
  'apply': { ru: 'Подкоманда: применить без удаления из хранилища', en: 'Subcommand: apply without removing from storage' },
  'list': { ru: 'Подкоманда: показать список сохранённых записей', en: 'Subcommand: show list of saved entries' },
  'drop': { ru: 'Подкоманда stash: удалить запись из хранилища', en: 'Stash subcommand: delete entry from stash' },
  'start': { ru: 'Подкоманда bisect: начать поиск бага', en: 'Bisect subcommand: start bug search' },
  'bad': { ru: 'Подкоманда bisect: пометить коммит как содержащий баг', en: 'Bisect subcommand: mark commit as containing the bug' },
  'good': { ru: 'Подкоманда bisect: пометить коммит как рабочий', en: 'Bisect subcommand: mark commit as working' },
  'set-url': { ru: 'Подкоманда remote: изменить URL удалённого сервера', en: 'Remote subcommand: change URL of remote server' },
  'remove': { ru: 'Подкоманда: удалить запись (remote/stash)', en: 'Subcommand: remove entry (remote/stash)' },
  'update': { ru: 'Подкоманда: обновить (submodule / remote)', en: 'Subcommand: update (submodule / remote)' },
  'foreach': { ru: 'Подкоманда submodule: выполнить команду в каждом модуле', en: 'Submodule subcommand: run command in each module' },
  'lfs': { ru: 'Подкоманда: Large File Storage — хранение больших файлов', en: 'Subcommand: Large File Storage — storing large files' },
  'install': { ru: 'Подкоманда: установить/инициализировать', en: 'Subcommand: install/initialize' },
  'track': { ru: 'Подкоманда lfs: отслеживать файлы по шаблону', en: 'LFS subcommand: track files by pattern' },
  'filter-repo': { ru: 'Подкоманда: переписать историю (удалить файлы из всех коммитов)', en: 'Subcommand: rewrite history (remove files from all commits)' },
  'prune': { ru: 'Подкоманда: удалить устаревшие/недостижимые объекты', en: 'Subcommand: remove stale/unreachable objects' },
  // Extra flags
  '--version': { ru: 'Флаг: показать версию Git', en: 'Flag: show Git version' },
  '-c': { ru: 'Флаг: создать новую ветку и переключиться на неё', en: 'Flag: create new branch and switch to it' },
  '--force-with-lease': { ru: 'Флаг: принудительный push, но с проверкой что никто не запушил после тебя', en: 'Flag: force push but check that nobody pushed after you' },
  '--ff-only': { ru: 'Флаг: выполнить только если возможен fast-forward (без merge-коммита)', en: 'Flag: only proceed if fast-forward is possible (no merge commit)' },
  '--autostash': { ru: 'Флаг: автоматически спрятать и вернуть незакоммиченные изменения', en: 'Flag: auto-stash and restore uncommitted changes' },
  '--no-rebase': { ru: 'Флаг: использовать merge вместо rebase при pull', en: 'Flag: use merge instead of rebase when pulling' },
  '--autosquash': { ru: 'Флаг: автоматически сжать fixup-коммиты при rebase', en: 'Flag: auto-squash fixup commits during rebase' },
  '--onto': { ru: 'Флаг: перенести коммиты на указанную ветку-базу', en: 'Flag: transplant commits onto a specified base branch' },
  '--fixup': { ru: 'Флаг: создать коммит-исправление для последующего autosquash', en: 'Flag: create a fixup commit for later autosquash' },
  '-s': { ru: 'Флаг: добавить Signed-off-by (подпись автора)', en: 'Flag: add Signed-off-by (author sign-off)' },
  '--signoff': { ru: 'Флаг: добавить строку Signed-off-by к коммиту', en: 'Flag: add Signed-off-by line to commit' },
  '--tags': { ru: 'Флаг: включить теги в операцию (push/fetch)', en: 'Flag: include tags in operation (push/fetch)' },
  '--delete': { ru: 'Флаг: удалить ветку/тег на удалённом сервере', en: 'Flag: delete branch/tag on remote server' },
  '--branch': { ru: 'Флаг: указать конкретную ветку для клонирования', en: 'Flag: specify a particular branch to clone' },
  '--single-branch': { ru: 'Флаг: клонировать только одну ветку (экономит место)', en: 'Flag: clone only one branch (saves space)' },
  '--detach': { ru: 'Флаг: переключиться без привязки к ветке (detached HEAD)', en: 'Flag: switch without attaching to a branch (detached HEAD)' },
  '--merged': { ru: 'Флаг: показать только ветки, которые уже влиты', en: 'Flag: show only branches already merged' },
  '--no-merged': { ru: 'Флаг: показать ветки, которые ещё не влиты', en: 'Flag: show branches not yet merged' },
  '--author': { ru: 'Флаг: фильтр по автору коммитов', en: 'Flag: filter by commit author' },
  '--since': { ru: 'Флаг: показать коммиты после указанной даты', en: 'Flag: show commits after specified date' },
  '--until': { ru: 'Флаг: показать коммиты до указанной даты', en: 'Flag: show commits before specified date' },
  '--grep': { ru: 'Флаг: искать по тексту в сообщениях коммитов', en: 'Flag: search by text in commit messages' },
  '--name-only': { ru: 'Флаг: показать только имена изменённых файлов', en: 'Flag: show only names of changed files' },
  '--word-diff': { ru: 'Флаг: показать разницу пословно (а не построчно)', en: 'Flag: show diff word-by-word (not line-by-line)' },
  '--no-color': { ru: 'Флаг: убрать цветную подсветку из вывода', en: 'Flag: remove color from output' },
  '--include-untracked': { ru: 'Флаг: включить неотслеживаемые файлы в stash', en: 'Flag: include untracked files in stash' },
  '--keep-index': { ru: 'Флаг: спрятать изменения, но оставить staged-файлы', en: 'Flag: stash changes but keep staged files' },
  '-x': { ru: 'Флаг clean: удалить также игнорируемые файлы', en: 'Flag clean: also remove ignored files' },
  '-X': { ru: 'Флаг clean: удалить ТОЛЬКО игнорируемые файлы', en: 'Flag clean: remove ONLY ignored files' },
  '--list': { ru: 'Флаг: показать все значения/записи списком', en: 'Flag: show all values/entries as a list' },
  '--unset': { ru: 'Флаг config: удалить настройку', en: 'Flag config: remove a setting' },
  '-l': { ru: 'Флаг: вывести список (теги/настройки)', en: 'Flag: list entries (tags/settings)' },
  '--init': { ru: 'Флаг submodule: инициализировать при обновлении', en: 'Flag submodule: initialize during update' },
  '--recursive': { ru: 'Флаг: применить рекурсивно ко всем вложенным', en: 'Flag: apply recursively to all nested items' },
  '--invert-paths': { ru: 'Флаг filter-repo: исключить указанные пути (а не оставить)', en: 'Flag filter-repo: exclude specified paths (instead of keeping)' },
  '--path': { ru: 'Флаг filter-repo: указать путь для фильтрации', en: 'Flag filter-repo: specify path for filtering' },
  '--renormalize': { ru: 'Флаг add: пересчитать line endings для всех файлов', en: 'Flag add: re-normalize line endings for all files' },
  '--source': { ru: 'Флаг restore: указать коммит-источник для восстановления', en: 'Flag restore: specify source commit for restoration' },
  '-A': { ru: 'Флаг add: добавить все файлы (новые + изменённые + удалённые)', en: 'Flag add: stage all files (new + modified + deleted)' },
  '--update': { ru: 'Флаг add: добавить только изменённые и удалённые (без новых)', en: 'Flag add: stage only modified and deleted (not new)' },
  '--no-commit': { ru: 'Флаг: выполнить merge/cherry-pick без автоматического коммита', en: 'Flag: perform merge/cherry-pick without auto-committing' },
  '--strategy': { ru: 'Флаг: выбрать стратегию слияния', en: 'Flag: choose merge strategy' },
  '--ours': { ru: 'Флаг/стратегия: при конфликте оставить нашу версию', en: 'Flag/strategy: on conflict keep our version' },
  '--theirs': { ru: 'Флаг/стратегия: при конфликте оставить чужую версию', en: 'Flag/strategy: on conflict keep their version' },
  '-fd': { ru: 'Флаги clean: принудительно + папки', en: 'Flags clean: force + directories' },
  '-fdx': { ru: 'Флаги clean: принудительно + папки + игнорируемые файлы', en: 'Flags clean: force + directories + ignored files' },
  '-fX': { ru: 'Флаги clean: принудительно удалить только игнорируемые', en: 'Flags clean: force remove only ignored files' },
  // Arguments / values
  'upstream': { ru: 'Имя удалённого сервера (обычно оригинал форка)', en: 'Remote server name (usually the fork original)' },
  'origin/main': { ru: 'Ветка main на удалённом сервере origin', en: 'Branch main on remote server origin' },
  'develop': { ru: 'Ветка для разработки (convention)', en: 'Development branch (convention)' },
  'input': { ru: 'Значение: преобразовать CRLF→LF при коммите (для Mac/Linux)', en: 'Value: convert CRLF→LF on commit (for Mac/Linux)' },
  'ssh': { ru: 'Значение: использовать SSH-ключ для подписи', en: 'Value: use SSH key for signing' },
  'FETCH_HEAD': { ru: 'Ссылка на последний скачанный коммит с сервера', en: 'Reference to the last fetched commit from server' },
  // gh CLI
  'gh': { ru: 'GitHub CLI — управление GitHub из терминала', en: 'GitHub CLI — manage GitHub from terminal' },
  'pr': { ru: 'Подкоманда gh: работа с Pull Request', en: 'gh subcommand: work with Pull Requests' },
  'create': { ru: 'Подкоманда: создать (PR/release/issue)', en: 'Subcommand: create (PR/release/issue)' },
  'view': { ru: 'Подкоманда: просмотреть детали', en: 'Subcommand: view details' },
  'run': { ru: 'Подкоманда gh: работа с GitHub Actions', en: 'gh subcommand: work with GitHub Actions' },
  'release': { ru: 'Подкоманда gh: управление релизами', en: 'gh subcommand: manage releases' },
  'edit': { ru: 'Подкоманда: редактировать существующий ресурс', en: 'Subcommand: edit existing resource' },
  '--fill': { ru: 'Флаг gh: заполнить PR заголовком и описанием из коммитов', en: 'Flag gh: fill PR title and description from commits' },
  '--base': { ru: 'Флаг gh: указать базовую ветку для PR', en: 'Flag gh: specify base branch for PR' },
  '--web': { ru: 'Флаг gh: открыть в браузере', en: 'Flag gh: open in browser' },
  '--generate-notes': { ru: 'Флаг gh: авто-генерировать описание релиза из коммитов', en: 'Flag gh: auto-generate release notes from commits' },
  '--log-failed': { ru: 'Флаг gh: показать логи упавших шагов', en: 'Flag gh: show logs of failed steps' },
  '--draft': { ru: 'Флаг: создать как черновик (PR/release)', en: 'Flag: create as draft (PR/release)' },
}
...
;(function attachTokenTooltips() {
  ...
  codeEls.forEach((el) => {
    ...
    // Only process elements that contain "git" or "gh" keyword
    if (!/\b(git|gh)\b/.test(raw)) return
